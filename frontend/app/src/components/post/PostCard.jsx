...
export default class PostCard extends Component {
    ...
    handleUpdateComment = () => {
        const { editingComment, content } = this.state;
        const updatedComment = {
            ...editingComment,
            description: content
        }

        PostDataService.updateComment(this.state.target, this.props.post.id, updatedComment).then((res, err) => {
            if(err) {
                console.error("err comment", err);
            }
            this.refreshComments();
    ...
    handleDeleteComment = (comment) => {
        PostDataService.deleteComment(this.state.target, this.props.post.id, comment.id).then((res, err) => {
            if(err) {
                console.error("err comment", err);
            }
            this.refreshComments();
            ...
            <div className="ui-block ui-custom" key={this.props.post.id}>
                ...
                <div className="comments">
                    <div className="commentHolder" ref="comments">
                        {this.state.comments.map((comment, i) => (
                            <div className="comment" key={i}>
                                ...
                                {comment.username == AuthenticationService.getLoggedInUserName() ? 
                                    <div className="comment-buttons">
                                        <button className="btn btn-sm btn-danger" onClick={() => this.handleDeleteComment(comment)}>Delete</button>
                                        <button className="btn btn-sm btn-primary" onClick={() => this.handleEditComment(comment)}>Edit</button>
                                    </div>
                                    : ""
                                }
                            </div>
                    ...
                    <div className="comment-control form-row">
                        ...
                        />
                        <button 
                            className="btn btn-primary btn-status col-md-2 col-sm-2 col-xs-2" 
